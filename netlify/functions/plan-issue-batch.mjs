...
export default async(request)=>{
  try{
    ...
    const batch=Number(d.batch||1),label=String(d.batchLabel||`Batch ${batch}`),brief=String(d.batchBrief||'Produce five distinct article decisions.');
    ...
    if(articles.length!==5)return json(502,{ok:false,error:`Planner batch returned ${articles.length} usable articles; expected 5.`});
