...
export default async(request)=>{
  try{
    ...
    const existing=(Array.isArray(d.existingArticles)?d.existingArticles:[]).slice(0,45).map(x=>({id:String(x.id||''),title:String(x.title||''),purpose:String(x.purpose||'').slice(0,180),freshness:String(x.freshness||''),topic:String(x.topic||''),proof:String(x.proof||'').slice(0,160),history_status:String(x.history_status||''),history_match:String(x.history_match||''),history_publication:String(x.history_publication||''),history_score:Number(x.history_score||0)})).filter(x=>x.id&&x.title);
    const inventory=existing.map((x,i)=>`${i+1}. ${x.id} | ${x.title} | ${x.purpose} | ${x.freshness} | ${x.topic} | HISTORY=${x.history_status||'UNKNOWN'}${x.history_publication?` | MATCH=${x.history_publication}: ${x.history_match} (${x.history_score}%)`:''}`).join('\n');
...
- Recently covered ideas should have a strong presumption against REFRESH unless the answer has materially changed.
 CREATE NEW for genuinely new/current opportunities. Human UK wording. Include local proof/evidence needs. Do not invent businesses, facts, partners, events or reader quotes. For contentious angles include a fair countercase.\n\nSTRICT JSON ONLY:\n{"issue_summary":"","articles":[{"mode":"REUSE|LOCALISE|REFRESH|CREATE NEW","existing_article_id":"","title":"","question":"","problem":"","hook":"","reader":"","value":"","local_proof":"","evidence":"","life_lane":"Home & Property|Home Improvement & Garden|Money & Household Costs|Family & Children|Health & Wellbeing|Food & Dining|Pets & Animals|Motoring & Transport|Travel, Days Out & Experiences|Leisure, Culture & Entertainment|Community & Local Change|Work, Business & Opportunity|Open","lane":"Authority|Feature Partner|Activation|Community|Editorial|Open","partner_path":"","cta_type":"None|Reply|Comment|Save|Nominate|Button|Ask Expert|Booking|Directory","cta_text":"","stance":"PRACTICAL|NEUTRAL|CHALLENGE|CONTRARIAN|DEBATE|UNFILTERED","why_now":"","countercase":"","source_signal":""}]}`;
    ...
    let response,result,articles=[],dupHits=[];
    for(let attempt=1;attempt<=2;attempt++){
      const retryNote=attempt===1?'':`\n\nRETRY REQUIRED: your previous attempt matched recently published target-publication topics. Avoid them completely: ${dupHits.map(x=>x.title).join(' | ')}`;
      response=await createResponse({input:prompt+retryNote,useWeb:false,model,timeoutMs:60000});
      result=parseJsonText(outputText(response));
      articles=(Array.isArray(result.articles)?result.articles:[]).slice(0,5).map(a=>{
      ...
      dupHits=articles.map(a=>recentDuplicate(a,blockedRecentHistory)).filter(Boolean);
      if(articles.length===requestedCount&&!dupHits.length)break;
    }
    ...
    if(dupHits.length)return json(409,{ok:false,error:`Recent-history guard rejected ${dupHits.length} duplicate candidate${dupHits.length===1?'':'s'} after retry: ${dupHits.map(x=>x.title).join(' | ')}. Click Build / Resume Issue Plan again; completed clean batches remain saved.`});
