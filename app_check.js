...
async function runProduction({one=false,mode="generate"}={}){
 ...
 if(one){
   ...
   let queue;
   try{queue=await productionSections()}catch(e){alert(e.message);return}
...
function workspaceReportV3186(){
  ...
  try{
    if(typeof persistentSupportRecordsV3153==='function'){
      const supports=persistentSupportRecordsV3153().slice().sort((a,b)=>Number(val(a.fields||{},'Section Order')||999)-Number(val(b.fields||{},'Section Order')||999));
...
function livePublishedHistoryEntries(publication=currentPublicationName()){
 const rows=[];
 const add=(record)=>{
   if(!record||!isProducedArticle(record))return;
...
function makeArticleBlock(s){const blockUid=uid();return {uid:blockUid,componentId:'CMP-'+blockUid.slice(-10).toUpperCase(),kind:'article',refId:s.id,type:'MASTER ARTICLE',title:val(s.fields,'Section Title')||'Untitled article',purpose:val(s.fields,'Reader Value')||val(s.fields,'Core Reader Question')||'',partnerAttached:false,partner:'',category:'',suggestedPartnerCategory:val(s.fields,'Potential Sponsor Category')||'',commitment:'',cta:val(s.fields,'CTA Text')||val(s.fields,'Primary Next Action')||'',button:val(s.fields,'CTA Text')||'',url:articleUrl(s),proof:val(s.fields,'Local Proof Needed')||val(s.fields,'Evidence Required')||'',content:val(s.fields,'Section Final Copy')||val(s.fields,'Final Copy')||'',lifeLane:val(s.fields,'Commercial Lane')||'',commercialRole:'EDITORIAL',productionTemplate:'MASTER ARTICLE',productionPoints:5,contentStatus:'APPROVED',productionStatus:(articleUrl(s)?'ASSETS NEEDED':'ASSETS NEEDED'),status:'READY'}}
...
function renderAssembly(){
 ...
 lib.innerHTML=rows.length?rows.map(s=>{const x=inferArticleIntelligence(s),title=String(val(s.fields,'Section Title')||'').trim().toLowerCase().replace(/\s+/g,' '),added=getCanvas().some(b=>b.kind==='article'&&(b.refId===s.id||String(b.title||'').trim().toLowerCase().replace(/\s+/g,' ')===title));return `<div class="palette-item" draggable="${added?'false':'true'}" ${added?'style="opacity:.55;cursor:default"':'ondragstart="startPaletteDrag(event,\'article\',\''+s.id+'\')"'} ondblclick="${added?'':'addArticleToCanvas(\''+s.id+'\')'}"><strong>${esc(val(s.fields,'Section Title')||'Untitled')}</strong><small>${added?'Already in Canvas':(masterArticleState(s)==='complete'?'MASTER ARTICLE COMPLETE ✓ · ':masterArticleReviewLabel(s)+' · ')+esc(x.topic)+' · '+esc(x.freshness)+' · drag or double-click'}</small></div>`}).join(''):`<div class="empty">${produced.length?'No produced articles match this search.':'No produced Master Articles are available for this publication yet.'}</div>`;if(E('assembly-library-diag'))E('assembly-library-diag').textContent=S.articleLibraryDiag||'';
 ...
 updatePlannerScore();renderComponentCommandCentre();renderInspector();renderSmartPlan();
}
function renderComponentCommandCentre(){
 ...
 if(E('command-summary'))E('command-summary').textContent=total?`${score}% through the full component lifecycle · ${items.filter(x=>x.kind==='article').length} Master Articles · ${items.filter(x=>x.kind!=='article').length} supporting components.`:'Add or assemble components to begin.';
...
function currentSupportComponentsV315(){
  return persistentSupportRecordsV3153().map(s=>{
...
function supportTargetV315(){
  ...
  return {
    ...
    supportNeeded:Math.max(0,issueTarget-existingMasters)
  };
}
...
function supportAllocationPlanV31814(profile, recipe, plan, selectedOrders){
  ...
  const score=(type,c)=>{
    ...
    if(/event|what's on|weekend/.test(t))n+=/(event|festival|market|opening|launch|weekend|date|show|fair)/.test(b)?8:-8;
...
async function buildSupportBatchV315(){
  ...
  try{
    ...
    if(!pending.length){
      setStep6LiveProgressV3152({show:true,done:total,total,ready:readyNow,failed:0,current:'All supporting components are already saved and ready in Airtable.',phase:'Step 6 complete'});
...
async function renderStep6StatusV3151(){
  ...
  if(btn){
    btn.textContent=remaining>0?'Build / Resume Supporting Components':
      (ready>=t.supportNeeded?'Go To Step 7 — Assemble':'Produce Remaining Components');
  }

  const headline=ready>=t.supportNeeded&&t.supportNeeded>0
    ? `Step 6 complete · ${built}/${t.supportNeeded} built · ${ready} ready · 0 remaining`
    : `${built}/${t.supportNeeded} support components built · ${ready} ready`;
  const message=ready>=t.supportNeeded&&t.supportNeeded>0
    ? 'Step 6 is complete. All supporting components are persistently saved. Next: Step 7 — Assemble.'
...
function renderStep6SummaryV315(){
  ...
  <div class="muted" style="margin-top:6px">Selected Masters: ${t.selectedMasters} · Target issue size: ${t.issueTarget} · Support/partner components required: ${t.supportNeeded}</div>
...
function workflowStepInfoV312(step){
 ...
 5:['Select',`Choose ${min}–${max} strongest READY Masters for this issue. ICS recommends from the actual Step 4 production states and editorial grades; remaining READY candidates stay bankable.`,`Review Master Selection`],
...
function updateWorkflowShellV312(forcedStep){
 ...
 document.querySelectorAll('[data-flow-step]').forEach(b=>{const n=Number(b.dataset.flowStep);b.classList.toggle('active',n===workflowSelectedStepV312);b.classList.toggle('done',n<auto)});
...
function runWorkflowPrimaryV312(){
 ...
 if(step===4){show('build');setTimeout(()=>E('run-production')?.click(),80);return}
 if(step===5){
   show('assemble');
...
function buildSmartCanvas(articles,persistentSupports=null){
  ...
  const supports=(persistentSupports&&persistentSupports.length)?persistentSupports.map(s=>supportRecordToCanvasV3153(s)):(profile.supportRecipe||[]).map(type=>comp(type));
  ...
  const items=[];let ai=0,si=0;
  if(supports.length)items.push(supports[si++]);
  while(ai<articles.length||si<supports.length){
    if(ai<articles.length){const a=articles[ai++];a.lifeLane=editorialLaneFor(a,profile);items.push(a)}
    if(si<supports.length)items.push(supports[si++]);
    if(si<supports.length && (items.length%5===0||ai>=articles.length))items.push(supports[si++]);
  }
  const max=profile.targetSections[1];
...
async function smartAssembleIssue(){
  ...
  const persistedSupport=persistentSupportRecordsV3153().filter(supportReadyRecordV3153);
  ...
  setStep7StatusV3164('Rebuilding Running Order…',
    `${chosen.length} clean READY Masters + ${requiredSupport} persistent support components. Discarding the old browser running order.`);
  ...
  setStep7StatusV3164(`Running Order Ready — ${items.length}/${items.length}`,
    `${chosen.length} Masters + ${items.length-chosen.length} support/partner components. Built fresh from the current saved Step 5 selection and persistent Step 6 records.`);
}
...
function deterministicQA(items){
  ...
  const articleCount=items.filter(b=>b.kind==='article').length;
  ...
  const longArticles=items.filter(b=>b.kind==='article'&&String(b.content||'').trim().split(/\s+/).filter(Boolean).length>650);
  if(longArticles.length)add('WARNING','MEGA_ARTICLE',`${longArticles.length} Master Article${longArticles.length===1?' is':'s are'} over about 650 words. Run the Split Test: does each contain two or more questions that deserve separate 250-600 word answers?`,longArticles.map(b=>b.uid));
  else add('PASS','ARTICLE_LENGTH',`No obvious mega-articles over about 650 words.`);
...
function loadSavedQAReport(){
  try{
    ...
    if(report?.qaVersion==='3.17.3.7')showQAReport(report,false);
    else clearQAReportView('Fresh QA required in v3.17.3.7. Run Final QA before fixing exceptions.');
  }catch{clearQAReportView('Fresh QA required in v3.17.3.7. Run Final QA before fixing exceptions.');}
}
...
function assemblyDraft(){
  const rawItems=getCanvas(),theme=String(E('assembly-theme')?.value||val(S.issue?.fields||{},'Main Theme')||'').trim();
  ...
  items.forEach((b,i)=>{
    ...
    if(isArticle)out+=`\n\n⚠ ARTICLE — BUILD SEPARATELY IN LETTERMAN\nThis is a permanent Master Article. Create/publish the article first, then use its published URL where the newsletter section links to the article.`;
    if(b.partner)out+=`\nPARTNER / ATTRIBUTION: ${String(b.partner).trim()}`;
    if(isArticle&&meta?.subhead)out+=`\n\nARTICLE SUBTITLE\n${meta.subhead}`;
    if(content)out+=`\n\n${content}`;
    if(isArticle&&meta){
      const record=(S.sections||[]).find(x=>x.id===b.refId),f=record?.fields||{};
      const articleUrl=articlePublishedUrlV31816(publication,meta.slug);
...
function masterAssetsPackageV3181(){
  ...
  const articles=(plan?.articles||[]).filter(a=>selected.has(Number(a.order))).sort((a,b)=>Number(a.order)-Number(b.order));
...
E('reconcile-smart-queue')?.addEventListener('click',()=>reconcileApprovedSmartQueue().catch(e=>alert('Could not reconcile queue: '+e.message)));
...
E('smart-plan-lock').onclick=lockSmartPlan;E('smart-plan-apply').onclick=applySmartPlanToBuild;E('smart-edit-save').onclick=saveSmartArticleEdit;E('smart-edit-cancel').onclick=()=>E('smart-edit-panel').style.display='none';E('smart-plan-hide').onclick=()=>E('smart-plan-panel').style.display='none';E('assembly-produce').onclick=produceSupportingComponents;E('assembly-partners').onclick=assignActivePartners;E('assembly-qa').onclick=runFinalQA;E('qa-fix-v317').onclick=fixQAExceptionsV317;E('qa-rerun-v317').onclick=runFinalQA;E('assembly-fix').onclick=applySafeFixes;E('qa-copy').onclick=copyQAReport;E('qa-hide').onclick=()=>E('qa-report-panel').style.display='none';E('assembly-generate').onclick=generateAssembly;
...
function applyWorkflowLayoutV312C(step){
  ...
  if(step===9){
    ...
    const masters=blocks.filter(b=>String(b.type||'').toUpperCase()==='MASTER ARTICLE').length;
    if(E('pr-sections-v312c'))E('pr-sections-v312c').textContent=String(blocks.length||0);
    if(E('pr-masters-v312c'))E('pr-masters-v312c').textContent=String(masters||0);
    if(E('pr-status-v312c'))E('pr-status-v312c').textContent=String(val(S.issue?.fields,'Issue Status')||'CHECK').replace('READY FOR LETTERMAN','PRODUCTION READY');
  }
