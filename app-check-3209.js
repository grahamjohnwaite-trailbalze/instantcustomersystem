...
// v3.20.9 — four Features remain the production target, but a single evidence-blocked Feature must not stall a healthy issue.
function featureMinimumForIssueV3209(){return Math.min(3,featureTargetForIssueV3204())}
function featureLayerReadyV3209(){return readyFeatureArticlesV3204().length>=featureMinimumForIssueV3209()}
...
function fallbackFeaturePlanV3206(payload={}){
  const pub=String(payload.publicationName||'Local').trim();
  const place=(pub.replace(/\b(?:Spotlight|Taste Trail|Pet Insider|Home Seller Insider|Business Pulse)\b/gi,'').trim()||pub);
  const promise=String(payload.issuePromise||'').trim();
  const isTaste=/taste trail/i.test(pub),isPet=/pet insider/i.test(pub),isHome=/home seller/i.test(pub);
  let features;
  if(isTaste){
    features=[
      {title:`The £15 ${place} Taste Test: What Can You Actually Get?`,question:`What enjoyable food or drink can a reader genuinely get around ${place} for about £15?`,reader_value:'A saveable value challenge with named current venues and real prices.',local_proof:'At least three current named venues, menus/prices and opening information.',evidence:'Official venue menus/websites or current primary listings.',life_lane:'Food & Hospitality',writing_mode:'COMPARISON / VALUE',cta_text:''},
      {title:`One ${place} Food Detour Worth Making`,question:`Which one current ${place} food or drink stop rewards going slightly out of the way?`,reader_value:'A tempting discovery with enough specifics to decide whether to go.',local_proof:'Named venue, exact location, signature item, current price and opening evidence.',evidence:'Official venue source plus current menu/listing.',life_lane:'Places & Discovery',writing_mode:'RECOMMENDATION / DISCOVERY',cta_text:''},
      {title:`Before Or After? Build A Better ${place} Night Out`,question:`What can readers pair with a meal or drink in ${place} to turn it into a fuller outing?`,reader_value:'A practical mini-itinerary rather than another isolated venue mention.',local_proof:'Named nearby venue/activity pairings, walk/travel practicality and current opening/event proof.',evidence:'Official venue/event sources.',life_lane:'Places & Discovery',writing_mode:'PRACTICAL SERVICE',cta_text:''},
      {title:`The ${place} Menu Detail People Argue About`,question:`What small food or drink detail around ${place} reliably divides opinion and what do local places actually offer?`,reader_value:'A fun, evidence-led talking point readers can share.',local_proof:'Named local examples and current menu evidence.',evidence:'Official menus/current primary sources.',life_lane:'Fun & Curiosity',writing_mode:'DEBATE / READER VOICE',cta_text:''}
    ];
  }else if(isPet){
    features=[
      {title:`The 10-Minute ${place} Pet Check Worth Doing This Week`,question:'What quick preventative pet check can an owner do safely at home and when should they seek professional help?',reader_value:'A practical, saveable checklist with clear limits.',local_proof:`A relevant ${place} service/resource plus authoritative veterinary guidance.`,evidence:'Authoritative veterinary/welfare source and current local service evidence.',life_lane:'Family & Community',writing_mode:'PRACTICAL SERVICE',cta_text:''},
      {title:`A ${place} Pet-Friendly Place Worth Knowing`,question:`Which current ${place} place is genuinely useful or enjoyable for pet owners?`,reader_value:'A named local discovery readers can actually use.',local_proof:'Named place, access rules, current opening and pet policy.',evidence:'Official venue/site source.',life_lane:'Places & Discovery',writing_mode:'RECOMMENDATION / DISCOVERY',cta_text:''},
      {title:'Pet Myth Or Money Saver?',question:'Which common pet-spending shortcut is genuinely sensible and which can backfire?',reader_value:'A money-saving comparison without false economy.',local_proof:`Current ${place} price/example where available plus authoritative care guidance.`,evidence:'Authoritative animal-care source and current local pricing where material.',life_lane:'Money & Consumer',writing_mode:'COMPARISON / VALUE',cta_text:''},
      {title:`The ${place} Pet Question Owners Keep Asking`,question:`What recurring practical pet question around ${place} deserves a short evidence-led answer?`,reader_value:'A concise answer owners can share with another owner.',local_proof:'Current local route/resource and authoritative guidance.',evidence:'Primary/local plus authoritative welfare/veterinary source.',life_lane:'Fun & Curiosity',writing_mode:'NEWS EXPLAINER',cta_text:''}
    ];
  }else if(isHome){
    features=[
      {title:`The £500 ${place} Home-Sale Question`,question:'Which small pre-sale spend is most likely to improve presentation without pretending to guarantee a higher sale price?',reader_value:'A practical value comparison for sellers.',local_proof:`Current ${place} service/price examples where available.`,evidence:'Current local pricing plus authoritative property guidance.',life_lane:'Home & Property',writing_mode:'COMPARISON / VALUE',cta_text:''},
      {title:`One ${place} Viewing Detail Buyers Notice Fast`,question:'What small viewing detail can change a buyer’s first impression and what can a seller realistically do about it?',reader_value:'A one-job practical fix.',local_proof:'Specific local housing context and credible guidance.',evidence:'Authoritative property source and local market context.',life_lane:'Home & Property',writing_mode:'PRACTICAL SERVICE',cta_text:''},
      {title:`The ${place} Seller Myth Worth Dropping`,question:'Which common seller assumption sounds sensible but does not hold up well?',reader_value:'A memorable myth-buster with practical consequences.',local_proof:'Current local context/data where relevant.',evidence:'Authoritative housing/property source.',life_lane:'Money & Consumer',writing_mode:'NEWS EXPLAINER',cta_text:''},
      {title:`What £20 Buys A ${place} Seller Before Photos`,question:'What can a seller do with a tiny budget before listing photos that is actually worth the effort?',reader_value:'A saveable micro-checklist.',local_proof:'Local/current examples where material.',evidence:'Credible property presentation guidance.',life_lane:'Home & Property',writing_mode:'PRACTICAL SERVICE',cta_text:''}
    ];
  }else{
    features=[
      {title:`The £10 ${place} Challenge: What Can You Actually Get?`,question:`What can someone genuinely do, eat, buy or enjoy around ${place} for about £10 right now?`,reader_value:'A fun, saveable local value test with named examples and real prices.',local_proof:`At least three current named ${place} examples with prices and practical details.`,evidence:'Official/current venue, retailer or service sources for prices and availability.',life_lane:'Money & Consumer',writing_mode:'COMPARISON / VALUE',cta_text:''},
      {title:`One ${place} Detour Worth Making`,question:`What current local place, small attraction, shop, walk or experience around ${place} is worth a deliberate detour?`,reader_value:'A tempting discovery that gives the reader enough detail to decide whether to go.',local_proof:'Named place, exact location, current opening/access details and one distinctive reason to visit.',evidence:'Official venue/organisation source plus current access information.',life_lane:'Places & Discovery',writing_mode:'RECOMMENDATION / DISCOVERY',cta_text:''},
      {title:`Remember This? A ${place} Detail Worth Bringing Back`,question:`What specific piece of ${place} local life, habit, place or tradition is worth remembering — and what exists there now?`,reader_value:'Nostalgia with a current payoff rather than vague reminiscing.',local_proof:'Named historical/local detail and a current status check.',evidence:'Credible local history/archive source plus current primary evidence.',life_lane:'Local Life',writing_mode:'HUMAN / COMMUNITY',cta_text:''},
      {title:`The ${place} Shortcut That Is Actually Worth Knowing`,question:`What small local shortcut, service, route, saving or practical trick around ${place} can genuinely make everyday life easier?`,reader_value:'A useful tip readers may save or send to someone else.',local_proof:'Named current local service, route, price, rule or resource.',evidence:'Official/current primary source for the practical claim.',life_lane:'Local Life',writing_mode:'PRACTICAL SERVICE',cta_text:''}
    ];
  }
  const count=Math.max(4,Math.min(6,Number(payload.count)||4));
  return {ok:true,features:features.slice(0,count),fallback:true,reason:'Feature planner endpoint did not return a completed 200 response.'};
}
async function apiFeaturePlanV3206(payload,btn){
  let last;
  for(let attempt=1;attempt<=2;attempt++){
    try{
      ...
      last=new Error(d?.error||`Request failed (${r.status})`);
      if([202,408,429,500,502,503,504].includes(r.status)){
        if(btn)btn.textContent=`Planner unavailable (${r.status}) — using safe fallback…`;
        break;
      }
      throw last;
    }catch(e){
      ...
      if(attempt<2){if(btn)btn.textContent='Feature planner retry…';await sleep(800);continue}
    }
  }
  console.warn('Feature planner fallback engaged',last);
  return fallbackFeaturePlanV3206(payload);
}
...
async function renderStep6StatusV3151(){
  ...
  if(detail){
    detail.textContent=
      ...
      `Feature Articles ready: ${t.featureArticles}/${featureTargetForIssueV3204()} (minimum ${featureMinimumForIssueV3209()} to assemble)\n`+
  ...
  const featuresReady=t.featureArticles>=featureMinimumForIssueV3209();
...
function persistedAssemblyCompleteV31821(){
  ...
  const expectedFeatureIds=new Set(readyFeatureArticlesV3204().map(x=>String(x.id)));
  const gotFeatureIds=new Set(features.map(x=>String(x.s.id)));
  if(masters.length!==selected.length||features.length!==expectedFeatureIds.size||supports.length<supportNeed)return false;
  if(expectedFeatureIds.size<featureMinimumForIssueV3209()||![...expectedFeatureIds].every(id=>gotFeatureIds.has(id)))return false;
...
function legacyAssemblyStructurallyCompleteV3191(){
  ...
  const expectedFeatureIds=new Set(readyFeatureArticlesV3204().map(x=>String(x.id)));
  const gotFeatureIds=new Set(features.map(x=>String(x.s.id)));
  return masters.length>=minMasters&&masters.length<=maxMasters&&expectedFeatureIds.size>=featureMinimumForIssueV3209()&&features.length===expectedFeatureIds.size&&[...expectedFeatureIds].every(id=>gotFeatureIds.has(id))&&supports.length>=supportNeed;
}
...
function browserAssemblyStructurallyCompleteV3192(){
  ...
  const [minMasters,maxMasters]=profile.targetMasters||[1,99];
  const expectedFeatureIds=new Set(readyFeatureArticlesV3204().map(x=>String(x.id)));
  const gotFeatureIds=new Set(features.map(x=>String(x.refId||'')));
  if(masters.length<minMasters||masters.length>maxMasters||expectedFeatureIds.size<featureMinimumForIssueV3209()||features.length!==expectedFeatureIds.size||supports.length<supportNeed)return false;
  if(![...expectedFeatureIds].every(id=>gotFeatureIds.has(id)))return false;
...
function updateWorkflowShellV312(forcedStep){
 ...
 const expectedFeatureIdsV3209=new Set(readyFeatureArticlesV3204().map(x=>String(x.id)));
 const canvasFeatureIdsV3209=new Set(stateCanvasV31821.filter(b=>String(b.type||'').toUpperCase()==='FEATURE ARTICLE').map(b=>String(b.refId||'')));
 const canvasMasterCountV3209=stateCanvasV31821.filter(b=>b.kind==='article'&&String(b.type||'').toUpperCase()==='MASTER ARTICLE').length;
 const browserAssemblyOkV31821=selectionReady
   && expectedFeatureIdsV3209.size>=featureMinimumForIssueV3209()
   && stateCanvasV31821.length>=expectedSectionsV31819 && stateCanvasV31821.length<=maxSectionsV31819
   && canvasMasterCountV3209===selected.length
   && canvasFeatureIdsV3209.size===expectedFeatureIdsV3209.size
   && [...expectedFeatureIdsV3209].every(id=>canvasFeatureIdsV3209.has(id))
   && canvasSupportsV31819.length>=targetV315.supportNeeded
   && selectedMastersPresentV31819
   && assemblyRhythmIntegrityV3204(stateCanvasV31821).ok;
...
// v3.20.9 — Step 7 must be built from current Step 5 + Step 6A + Step 6 state, never a stale browser/persisted order.
function assemblyCurrentInputsIntegrityV3209(items=getCanvas()){
  const plan=getSmartPlan(),selected=new Set(getMasterSelectionV312().map(Number));
  const expectedMasterIds=new Set((plan?.articles||[]).filter(a=>selected.has(Number(a.order))).map(a=>planArticleRecordV312(a)?.id).filter(Boolean).map(String));
  const expectedFeatureIds=new Set(readyFeatureArticlesV3204().map(x=>String(x.id)));
  const requiredSupport=supportTargetV315().supportNeeded;
  const expectedSupportIds=new Set(supportRecordsForTargetV3204(persistentSupportRecordsV3153(),requiredSupport).filter(supportReadyRecordV3153).map(x=>String(x.id)));
  const gotMasterIds=new Set((items||[]).filter(b=>b.kind==='article'&&String(b.type||'').toUpperCase()==='MASTER ARTICLE').map(b=>String(b.refId||'')));
  const gotFeatureIds=new Set((items||[]).filter(b=>String(b.type||'').toUpperCase()==='FEATURE ARTICLE').map(b=>String(b.refId||'')));
  const gotSupportIds=new Set((items||[]).filter(b=>b.kind!=='article').map(b=>String(b.refId||'')));
  const errors=[];
  if(expectedFeatureIds.size<featureMinimumForIssueV3209())errors.push(`Only ${expectedFeatureIds.size} READY Feature Articles; minimum is ${featureMinimumForIssueV3209()}.`);
  if(gotMasterIds.size!==expectedMasterIds.size||![...expectedMasterIds].every(id=>gotMasterIds.has(id)))errors.push('Running order does not match the current Step 5 Master selection.');
  if(gotFeatureIds.size!==expectedFeatureIds.size||![...expectedFeatureIds].every(id=>gotFeatureIds.has(id)))errors.push(`Running order omits or contains stale Feature Articles (${gotFeatureIds.size}/${expectedFeatureIds.size} current Features present).`);
  if(gotSupportIds.size!==expectedSupportIds.size||![...expectedSupportIds].every(id=>gotSupportIds.has(id)))errors.push(`Running order does not match current Step 6 support (${gotSupportIds.size}/${expectedSupportIds.size} current supports present).`);
  const rhythm=assemblyRhythmIntegrityV3204(items||[]);if(!rhythm.ok)errors.push(...rhythm.errors);
  return {ok:errors.length===0,errors,expectedMasterIds,expectedFeatureIds,expectedSupportIds};
}
let step7AutoRepairRunningV3209=false;
async function ensureCurrentStep7AssemblyV3209(){
  if(step7AutoRepairRunningV3209||!S.issue)return;
  try{
    await Promise.all([loadSections(),loadArticleLibrary()]);
    const check=assemblyCurrentInputsIntegrityV3209(getCanvas());
    if(check.ok){setStep7StatusV3164('Running Order Current ✓','Step 7 matches the current Masters, READY Features and support components; rhythm integrity passed.');return;}
    step7AutoRepairRunningV3209=true;
    setStep7StatusV3164('Refreshing Running Order…',check.errors.join(' ')+' ICS is rebuilding Step 7 from current saved state; nothing earlier will be changed.');
    await smartAssembleIssue();
  }finally{step7AutoRepairRunningV3209=false;}
}
...
async function smartAssembleIssue(){
  ...
  if(features.length<featureMinimumForIssueV3209()){
    setStep7StatusV3164('Step 7 blocked — Feature layer too thin.',`${features.length}/${featureTargetForIssueV3204()} Feature Articles are READY. At least ${featureMinimumForIssueV3209()} are required to create a balanced running order; finish or recommission Step 6A.`);
    return;
  }
  ...
  const currentIntegrity=assemblyCurrentInputsIntegrityV3209(items);
  if(!currentIntegrity.ok){
    clearRunningOrderV3164();
    setStep7StatusV3164('Step 7 failed current-state integrity.',currentIntegrity.errors.join(' '));
    return;
  }
  setStep7StatusV3164(`Running Order Ready — ${items.length}/${items.length} · current + rhythm passed`,
    `${chosen.length} Masters + ${features.length} Feature Articles + ${items.length-chosen.length-features.length} support/partner components. All READY Features are included, no stale support remains, and no support-only tail is allowed.`);
}
...
function focusWorkflowStepV312B(step,{scroll=true}={}){
  ...
  if(step===7)setTimeout(()=>ensureCurrentStep7AssemblyV3209().catch(e=>setStep7StatusV3164('Step 7 refresh failed.',String(e?.message||e))),0);
